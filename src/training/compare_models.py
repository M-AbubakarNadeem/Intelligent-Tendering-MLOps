...
from src.data.preprocess import create_dataloaders, NUM_LABELS, ID_TO_LABEL
...
from src.training.train import evaluate_sinsent, evaluate_seqsent
...
def plot_metric_comparison(sinsent_metrics, seqsent_metrics, output_dir):
    ...
    bars1 = ax.bar(x - width / 2, sinsent_vals, width, label="SinSent", color="#4C72B0", edgecolor="white")
    bars2 = ax.bar(x + width / 2, seqsent_vals, width, label="SeqSent", color="#DD8452", edgecolor="white")
...
def plot_confusion_matrices(sinsent_preds, sinsent_labels, seqsent_preds, seqsent_labels, output_dir):
...
def plot_confidence_distribution(sinsent_probs, seqsent_probs, output_dir):
    ...
    for ax, probs, title in [
        ...
        ax.axvline(x=np.median(max_probs), color="red", linestyle="--", label=f"Median: {np.median(max_probs):.3f}")
...
def plot_per_class_f1(sinsent_preds, sinsent_labels, seqsent_preds, seqsent_labels, output_dir):
...
def compare_models(args):
    ...
    print("-" * 55)
    for metric in ["f1_micro", "f1_macro", "f1_weighted", "precision_micro", "recall_micro"]:
    ...
    print(f"  SinSent: {sin_params['total_parameters']:,} total, {sin_params['trainable_parameters']:,} trainable")
    print(f"  SeqSent: {seq_params['total_parameters']:,} total, {seq_params['trainable_parameters']:,} trainable")

    # Latency benchmarking
    print(f"\n{'='*60}")
    print("LATENCY BENCHMARKING")
    print(f"{'='*60}")
    sinsent_latency = benchmark_latency(sinsent_model, sinsent_test_loader, device, "sinsent")
    seqsent_latency = benchmark_latency(seqsent_model, seqsent_test_loader, device, "seqsent")
    print(f"  SinSent: {sinsent_latency['mean_time']:.2f}s ± {sinsent_latency['std_time']:.2f}s")
    print(f"  SeqSent: {seqsent_latency['mean_time']:.2f}s ± {seqsent_latency['std_time']:.2f}s")
    ...
    plot_confusion_matrices(sinsent_preds, sinsent_labels, seqsent_preds, seqsent_labels, output_dir)
    plot_confidence_distribution(sinsent_probs, seqsent_probs, output_dir)
    plot_per_class_f1(sinsent_preds, sinsent_labels, seqsent_preds, seqsent_labels, output_dir)
